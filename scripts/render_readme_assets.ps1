...
$port = 5442
$process = $null
$stdout = Join-Path $env:TEMP ("kyc-refresh-case-router-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("kyc-refresh-case-router-" + [guid]::NewGuid().ToString() + "-stderr.log")
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/kyc-lane"; File = "02-kyc-lane-proof.png"; Size = "1600,1400" },
        @{ Url = "http://127.0.0.1:$port/refresh-queue"; File = "03-refresh-queue-proof.png"; Size = "1600,1320" },
